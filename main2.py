...
class LineNumbers(QWidget):
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        for _, block in iterate_viewport_blocks(self.editor):
            if block_top > event.rect().bottom():
                break
            # if block has a userdata.number, use that, else fall back to its displayed index+1
            data = block.userData()
            num = getattr(data, 'number', None)
            if num is None:
                num = block.blockNumber() + 1
            painter.drawText(0, block_top, font_width, font_height, Qt.AlignRight, str(num))
...
class DiffEditor(QWidget):
    ...
    def set_diff_text(self, old: str, new: str, lexer: Lexer):
        ...
        self.collapsed_section_starts = []  # List of (0‑based) start indices of each collapse
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            ...
            if tag == 'equal' and num_equal_lines > self.COLLAPSE_THRESHOLD_LINES:
                ...
                # Remember both the lines *and* the true start index
                start = i1 + self.COLLAPSE_CONTEXT_LINES
                self.collapsed_section_starts.append(start)
                self.collapsed_sections.append(
                    self.original_old[start : i2 - self.COLLAPSE_CONTEXT_LINES]
                )

                # Show top context
                for k in range(self.COLLAPSE_CONTEXT_LINES):
                    self.old_line_map.append(i1 + k + 1)
                    self.new_line_map.append(j1 + k + 1)
                ...
                # Add placeholder
                self.old_line_map.append(i1 + self.COLLAPSE_CONTEXT_LINES + 1)
                self.new_line_map.append(j1 + self.COLLAPSE_CONTEXT_LINES + 1)
                displayed_old_parts.append(placeholder)
                displayed_new_parts.append(placeholder)

                # Show bottom context
                for k in range(self.COLLAPSE_CONTEXT_LINES):
                    self.old_line_map.append((i2 - self.COLLAPSE_CONTEXT_LINES) + k + 1)
                    self.new_line_map.append((j2 - self.COLLAPSE_CONTEXT_LINES) + k + 1)
                ...
                for off in range(i2 - i1):
                    self.old_line_map.append(i1 + off + 1)
                for off in range(j2 - j1):
                    self.new_line_map.append(j1 + off + 1)
        ...
        # after setting the text, tag each visible block with its original line #
        def tag_blocks(editor, orig_map):
            doc = editor.document()
            for disp_idx, orig_line in enumerate(orig_map):
                block = doc.findBlockByLineNumber(disp_idx)
                if block.isValid():
                    data = QTextBlockUserData()
                    data.number = orig_line
                    block.setUserData(data)

        # orig_map is a list mapping each displayed index → original line index+1
        tag_blocks(self.old.editor, self.old_line_map)
        tag_blocks(self.new.editor, self.new_line_map)
    ...
    def expand_section(self, index):
        if not (0 <= index < len(self.collapsed_sections)):
            return

        original_lines = self.collapsed_sections[index]
        start_original_line_index = self.collapsed_section_starts[index]
        ...
        for editor in [self.old.editor, self.new.editor]:
            ...
            while block.isValid():
                if block.userState() == index + 1:
                    # Found the placeholder block.

                    # Get a cursor and perform a single, robust replacement operation.
                    cursor = QTextCursor(block)
                    cursor.beginEditBlock()
                    cursor.movePosition(QTextCursor.StartOfBlock)
                    cursor.movePosition(QTextCursor.EndOfBlock, QTextCursor.KeepAnchor)
                    cursor.movePosition(QTextCursor.NextCharacter, QTextCursor.KeepAnchor) # Select newline
                    cursor.insertText(text_to_insert) # This replaces the selection
                    cursor.endEditBlock()

                    # The 'block' reference is still valid and now points to the
                    # first line of the newly inserted text.
                    # We now re-tag it and all subsequent new lines.
                    current_block = block
                    for k, _ in enumerate(original_lines):
                        if not current_block.isValid():
                            break  # Should not happen, but good for safety

                        data = current_block.userData()
                        if data is None:  # A new block might not have userData yet
                            data = QTextBlockUserData()

                        # Set the correct original line number (1-based)
                        data.number = start_original_line_index + k + 1
                        current_block.setUserData(data)

                        current_block = current_block.next()

                    break # Done with this editor, move to the next

                block = block.next()

        # Remove the expanded section and update remaining userStates & starts
        del self.collapsed_sections[index]
        del self.collapsed_section_starts[index]
