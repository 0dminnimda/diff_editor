...
class LineNumbers(QWidget):
    ...
    def _build_cache(self):
        ...
        while block.isValid():
            ...
            # Skip padding lines (empty lines not part of a collapsed section)
            if text or state > 0:  # Not a padding line
            ...
            elif text:  # Only increment if not a padding line
...
class DiffEditor(QWidget):
    ...
    def set_diff_text(self, old: str, new: str, lexer: Lexer):
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            ...
            if tag == 'equal' and num_equal_lines > self.COLLAPSE_THRESHOLD_LINES:
                ...
                old_lines = self.original_old[i1:i2]
                new_lines = self.original_new[j1:j2]
                old_count = len(old_lines)
                new_count = len(new_lines)

                if tag == 'equal':
                    displayed_old_parts.extend(old_lines)
                    displayed_new_parts.extend(new_lines)
                elif tag == 'replace':
                    # For replacements, pad the shorter side with empty lines
                    max_count = max(old_count, new_count)
                    displayed_old_parts.extend(old_lines + ["\n"] * (max_count - old_count))
                    displayed_new_parts.extend(new_lines + ["\n"] * (max_count - new_count))
                elif tag == 'delete':
                    # For deletions, add padding to the new side
                    displayed_old_parts.extend(old_lines)
                    displayed_new_parts.extend(["\n"] * old_count)
                elif tag == 'insert':
                    # For insertions, add padding to the old side
                    displayed_old_parts.extend(["\n"] * new_count)
                    displayed_new_parts.extend(new_lines)

                old_line_num += max(old_count, new_count)
                new_line_num += max(old_count, new_count)
    ...
    def _get_padding_highlights(self, editor: CodeEditor) -> list[QTextEdit.ExtraSelection]:
        selections = []
        padding_format = QTextCharFormat()
        padding_format.setBackground(QColor(40, 40, 40, 100))  # Darker background for padding lines
        padding_format.setProperty(QTextFormat.FullWidthSelection, True)

        block = editor.editor.document().firstBlock()
        while block.isValid():
            if not block.text().strip() and block.userState() <= 0:  # Padding line
                selection = QTextEdit.ExtraSelection()
                selection.format = padding_format
                selection.cursor = QTextCursor(block)
                selections.append(selection)
            block = block.next()
        return selections
    ...
    def update_diff(self):
        ...
        # Get placeholder and padding highlights and combine them with diff highlights
        ...
        old_padding_highlights = self._get_padding_highlights(self.old)
        new_padding_highlights = self._get_padding_highlights(self.new)

        self.old.apply_highlights(old_highlights + old_placeholder_highlights + old_padding_highlights)
        self.new.apply_highlights(new_highlights + new_placeholder_highlights + new_padding_highlights)

    @Slot(int)
    def expand_section(self, index):
        if 0 <= index < len(self.collapsed_sections):
            self._expand_section(index)
            # Re-run set_diff_text to re-align everything
            self.set_diff_text(''.join(self.original_old), ''.join(self.original_new), self.old.highlighter.lexer)
